...
The search process forms a binary tree where each level corresponds to a variable assignment:
...
Each path from the root to a leaf represents a complete assignment of all variables. For example, the path $\text{Start} \rightarrow p=1 \rightarrow Q=0$ represents the assignment $\{p=\text{true}, Q=\text{false}\}$.
