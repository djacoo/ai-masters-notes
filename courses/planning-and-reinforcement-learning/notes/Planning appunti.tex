...
\section{Summary of the General Simplex Method for LRA}

\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=\textbf{General Simplex Method for LRA}]
...
\begin{itemize}
    \item \textbf{Input}: A set of LRA constraints
    \item \textbf{Transformation}: Convert the constraints into the general form 
    \[
        Ax = 0
    \]
    with bounds 
    \[
        l_i \leq x_i \leq u_i
    \]
    \item \textbf{Algorithm}: Use the simplex algorithm to find a feasible solution.
    \begin{itemize}
        \item Maintain a tableau with basic and non-basic variables
        \item Non-basic variables are at their bounds
        \item If a basic variable violates its bound, pivot to bring it back within the bounds
    \end{itemize}
    \item \textbf{Output}: 
    \begin{itemize}
        \item If the system is satisfiable, return \texttt{sat}
        \item If the system is unsatisfiable, return \texttt{unsat}
    \end{itemize}
\end{itemize}
\end{tcolorbox}

\subsection{Example of Initial Tableau Construction}

Let us consider a simple linear programming problem to illustrate the creation of the initial Simplex tableau.
    
\begin{example}[Simple Linear Programming Problem]
    \textbf{Problem}: Maximize the objective function $Z$
    \[
        Z = 3x + 2y
    \]
    ...
    \textbf{Step 2: Create the Initial Simplex Tableau}
    ...
    \[ 
        \begin{array}{c|cccccc|c}
            \hline
            \text{Base} & Z & x & y & s_1 & s_2 & \text{RHS} \\
            \hline
            Z & 1 & -3 & -2 & 0 & 0 & 0 \\
            \hline
            s_1 & 0 & 2 & 1 & 1 & 0 & 18 \\
    ...
    \textbf{Explanation of the Initial Tableau:}
    
    \textbf{Rows:}
    \begin{itemize}
        \item The first row corresponds to the objective function: $3x + 2y - Z = 0$. The coefficients of $x$ and $y$ are negative because we moved them to the left side of the equation.
        \item The second row corresponds to the first constraint: $2x + y + s_1 = 18$.
        \item The third row corresponds to the second constraint: $2x + 3y + s_2 = 42$.
    \end{itemize}
    
    \textbf{Columns:}
    \begin{itemize}
        \item The first column corresponds to the objective function value: $Z$.
        \item The second and third columns correspond to the problem variables: $x$ and $y$.
        \item The fourth and fifth columns correspond to the slack variables: $s_1$ and $s_2$.
        \item The last column corresponds to the right-hand side constant terms of the equations: $18$ and $42$.
    \end{itemize}
    
    \textbf{Basic and Non-Basic Variables:}
    \begin{itemize}
        \item In the initial tableau, the slack variables ($s_1$ and $s_2$) and $Z$ are the basic variables. A variable is basic if it has a column with a single 1 and all other entries in that column are 0. The labels on the left side of the tableau indicate the basic variables for that row.
        \item The non-basic variables are the problem variables: $x$ and $y$, and are initially set to 0.
    \end{itemize}
    
    \textbf{Initial Feasible Solution:}
...
\subsection{Example: Unsatisfiable System}

Let us consider another example that demonstrates how the simplex method detects unsatisfiability.

\begin{example}[Unsatisfiable Constraint System]
    \textbf{Problem}: Consider the following system of constraints:
    \begin{align*}
        x + 2y &\geq 1 \\
        2x + y &\geq 1 \\
        2x + 2y &\leq 1
    ...
    \begin{align*}
        x + 2y - s_1 &= 0 \quad \text{with } s_1 \geq 1 \\
        2x + y - s_2 &= 0 \quad \text{with } s_2 \geq 1 \\
        2x + 2y - s_3 &= 0 \quad \text{with } s_3 \leq 1
    \end{align*}
    
    \textbf{Variable Ordering}: $x < y < s_1 < s_2 < s_3$
    
    \textbf{Step 2: Initial Tableau}
    ...
    \begin{itemize}
        \item $\mathcal{N} = \{x, y\}$ (non-basic variables)
        \item $\mathcal{B} = \{s_1, s_2, s_3\}$ (basic variables)
    \end{itemize}
    ...
    \textbf{Step 3: Initial Assignment}
    ...
    The new sets are:
    \begin{itemize}
        \item $\mathcal{N} = \{s_1, y\}$ (non-basic variables)
        \item $\mathcal{B} = \{x, s_2, s_3\}$ (basic variables)
    \end{itemize}
    ...
    \textbf{Step 5: Second Pivot}
    
    We observe that $\alpha_2(s_3) = 2$ violates the constraint $s_3 \leq 1$. We need to decrease $\alpha_2(s_3)$. The variable $s_1$ is not suitable because its coefficient in the constraint for $s_3$ is positive; to decrease $\alpha_2(s_3)$, we would need to decrease $\alpha_2(s_1)$, but we cannot because $\alpha_2(s_1) = 1$ is equal to the lower bound for $s_1$. The variable $y$ is suitable: we can increase $y$ to decrease $s_3$ because $y$'s coefficient is negative in the $s_3$ row. We compute:
    \[
    ...
    After the pivot, the new assignment is:
    \begin{align*}
        \alpha_3(y) &= 0 + \frac{1}{2} = \frac{1}{2} \\
        \alpha_3(s_1) &= \alpha_2(s_1) = 1 \\
        \alpha_3(x) &= 1 - 2 \cdot \frac{1}{2} = 0 \\
        \alpha_3(s_2) &= 2 - 3 \cdot \frac{1}{2} = \frac{1}{2} \\
        \alpha_3(s_3) &= 2 - 2 \cdot \frac{1}{2} = 1
    \end{align*}
    
    The system of equations becomes:
    \begin{align*}
        2y &= 2s_1 - s_3 \quad \Rightarrow \quad y = s_1 - \frac{s_3}{2} \\
        x &= s_1 - 2\left(s_1 - \frac{s_3}{2}\right) = s_1 - 2s_1 + s_3 = -s_1 + s_3 \\
        s_2 &= 2s_1 - 3\left(s_1 - \frac{s_3}{2}\right) = 2s_1 - 3s_1 + \frac{3s_3}{2} = -s_1 + \frac{3s_3}{2}
    \end{align*}
    
    The new sets are:
    \begin{itemize}
        \item $\mathcal{N} = \{s_1, s_3\}$ (non-basic variables)
        \item $\mathcal{B} = \{x, s_2, y\}$ (basic variables)
    \end{itemize}
    ...
    \textbf{Step 6: Detection of Unsatisfiability}
    
    We observe that $\alpha_3(s_2) = \frac{1}{2}$ violates the constraint $s_2 \geq 1$. We need to increase $\alpha_3(s_2)$. 
    \begin{itemize}
        \item The variable $s_1$ has a negative coefficient, so to increase $s_2$ we would need to decrease $s_1$, but we cannot because $s_1$ is equal to its lower bound.
        \item The variable $s_3$ has a positive coefficient, so we could increase $s_3$ to increase $s_2$, but that is not possible because $s_3$ is equal to its upper bound.
    \end{itemize}
    
    Since no independent variable is suitable to increase $s_2$, the system is unsatisfiable. The algorithm returns \texttt{unsat}.
\end{example}

\begin{tcolorbox}[colback=yellow!10!white,colframe=yellow!75!black,title=\textbf{Pivoting Rule}]
The \textbf{pivoting rule} is a rule to choose the suitable non-basic variable to pivot to when there is more than one candidate.

The pivoting rule consists of using a fixed ordering on variables known as \textbf{Bland's rule}, which avoids cycles in iterations by always selecting the smallest variable (according to the ordering) that is suitable for pivoting.
\end{tcolorbox}
